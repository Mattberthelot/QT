...
{
    QString Modele,Date,NumeroSerie;
    int Capacité;
    float Sensibilité;
    QChar d;
    ...
    if (fichierIni.exists() && fichierIni.isFile())
    {
        ...
        Modele=paramEquilibreuse.value("CONFIG/MOdele").toString();
        Date=paramEquilibreuse.value("CONFIG/DATE").toString();
        NumeroSerie=paramEquilibreuse.value("CONFIG/NumeroSerie").toString();
        Capacité=paramEquilibreuse.value("CONFG/Capacité").toInt();
        Sensibilité=paramEquilibreuse.value("CONFIG/Sensibilité").toFloat();
    }
    else
    {
        // si le fichier n’existe pas, il est créé avec les valeurs par défaut
             ...
             paliers[PALIER_A].jauge.date= "9-22-1998";
...
void Experience::EnregistreFichierIni(const typePalier _paliers[1])
{
